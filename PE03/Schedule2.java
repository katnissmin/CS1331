...
public class Schedule {
    public static void main(String[] args) {
        ...
        int[] difficulties1 = computeDifficulties(schedule1, subjectList);
        for (int i = 0; i < schedule1.length; i++) {
            System.out.println(schedule1[i] + " " + difficulties1[i]);
        }

        System.out.println("Schedule 2:");
        int[] difficulties2 = computeDifficulties(schedule2, subjectList);
        for (int i = 0; i < schedule2.length; i++) {
            System.out.println(schedule2[i] + " " + difficulties2[i]);
        }
    ...
    public static int[] computeDifficulties(String[] schedule, String[] subjectList) {
        ...
        for (int i = 0; i < schedule.length; i++) {
            ...
            for (int j = 0; j < subjectList.length; j++) {
                if (subject.equals(subjectList[j])) {
                    subjectIndex = j;
                    break;
                }
            }
